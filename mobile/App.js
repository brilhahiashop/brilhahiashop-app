...
export default function App() {
  ...
  async function sendLoginEmail() {
    ...
    try {
      ...
      Alert.alert("Email enviado", "Enviámos um link de acesso para o teu email. Abre-o no telemóvel e depois volta à APP.");
    } catch (e) {
  ...
  function changeEmail() {
    ...
    if (!value || !value.includes("@")) return Alert.alert("Email inválido", "Introduz o novo email.");
    injectAccountUpdate({ email: value });
  }

  function changePassword() {
    if (newPassword.length < 12) return Alert.alert("Password curta", "Usa pelo menos 12 caracteres.");
  ...
  function onMessage({ nativeEvent }) {
    try {
      ...
      if (m.type === "auth-state") {
        setAuthenticated(!!m.authenticated);
        if (m.authenticated) setSettingsOpen(false);
      }
  ...
  const authWatcher = `
    (function(){
      if (window.__brilhahAuthWatcher) return true;
      window.__brilhahAuthWatcher = true;
      function report(){
        try {
          var app=document.getElementById('app');
          var auth=document.getElementById('auth');
          var logged=!!(app && !app.classList.contains('hidden'));
          window.ReactNativeWebView.postMessage(JSON.stringify({type:'auth-state',authenticated:logged}));
        }catch(e){}
      }
      report();
    ...
    <SafeAreaView style={styles.container}>
      ...
      {loadError ? (
        <View style={styles.error}>
          ...
          <Text style={styles.errorText}>Não foi possível ligar ao painel. Verifica a internet e tenta novamente.</Text>
          <TouchableOpacity style={styles.primary} onPress={() => setLoadError(false)}><Text style={styles.primaryText}>Tentar novamente</Text></TouchableOpacity>
          {!authenticated && <TouchableOpacity style={styles.secondary} onPress={() => setSettingsOpen(true)}><Text style={styles.secondaryText}>Definições de acesso</Text></TouchableOpacity>}
        </View>
      ) : (
        <WebView
          ref={webRef}
          ...
          startInLoadingState
          renderLoading={() => <View style={styles.loading}><ActivityIndicator size="large" /><Text style={styles.loadingText}>A ligar ao BRILHAH AI Manager…</Text></View>}
          injectedJavaScript={authWatcher}
          onLoadEnd={() => webRef.current?.injectJavaScript(authWatcher)}
          onMessage={onMessage}
          ...
          allowsBackForwardNavigationGestures pullToRefreshEnabled setSupportMultipleWindows={false}
        />
      )}

      <Modal visible={!authenticated && settingsOpen} animationType="slide" transparent onRequestClose={() => setSettingsOpen(false)}>
        <View style={styles.modalBackdrop}><View style={styles.modalCard}>
          <View style={styles.modalHead}><Text style={styles.modalTitle}>Definições de acesso</Text><TouchableOpacity onPress={() => setSettingsOpen(false)}><Text style={styles.close}>✕</Text></TouchableOpacity></View>
          <Text style={styles.label}>Email principal</Text>
          <TextInput style={styles.input} value={email} onChangeText={setEmail} autoCapitalize="none" keyboardType="email-address" />
          <TouchableOpacity style={styles.primary} disabled={busy} onPress={sendLoginEmail}><Text style={styles.primaryText}>{busy ? "A enviar…" : "Esqueci a password · Enviar email"}</Text></TouchableOpacity>
          <View style={styles.divider} />
          <Text style={styles.note}>Depois de entrares, email e password passam a ser geridos no menu Definições dentro da BRILHAH AI Manager.</Text>
          <Text style={styles.label}>Mudar email</Text>
          <TextInput style={styles.input} value={newEmail} onChangeText={setNewEmail} placeholder="novo@email.com" placeholderTextColor="#66758b" autoCapitalize="none" keyboardType="email-address" />
          <TouchableOpacity style={styles.secondary} onPress={changeEmail}><Text style={styles.secondaryText}>Guardar novo email</Text></TouchableOpacity>
          <Text style={styles.label}>Mudar password</Text>
          <TextInput style={styles.input} value={newPassword} onChangeText={setNewPassword} placeholder="Nova password" placeholderTextColor="#66758b" secureTextEntry />
          <TouchableOpacity style={styles.secondary} onPress={changePassword}><Text style={styles.secondaryText}>Guardar nova password</Text></TouchableOpacity>
        </View></View>
      </Modal>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  topBar:{minHeight:50,backgroundColor:"#07101d",borderBottomWidth:1,borderBottomColor:"#243551",paddingHorizontal:12,flexDirection:"row",alignItems:"center",justifyContent:"space-between"},
  ...
  modalTitle:{color:"#fff",fontSize:20,fontWeight:"800"},close:{color:"#fff",fontSize:20,padding:6},
